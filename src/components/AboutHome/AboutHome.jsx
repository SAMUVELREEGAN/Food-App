...
>
  ...
  {/* Content */}
  <div
    className="container mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row
    items-center lg:gap-8 xl:gap-16 relative"
  >
    <div className="w-full order-1 lg:order-2 space-y-8 sm:space-y-12 relative">
      <div className="space-y-4 sm:space-y-8 px-4 sm:px-0">
        <h2 className="text-3xl sm:text-4xl md:text-5xl xl:text-6xl font-bold leading-tight">
          ...
          <br />
          <span
            className="inline-block mt-2 sm:mt-4 text-2xl sm:text-3xl md:text-4xl
            opacity-90 font-light"
          >
            Where Flavors Dance &amp; Memories Bloom
          </span>
        </h2>
        <p className='text-base sm:text-xl opacity-90 leading-relaxed max-w-3xl font-serif
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 px-4 sm:px-0'>
        {aboutfeature.map((item,i) => (
            ...
            duration-300 p-4 sm:p-5 hover:translate-x-2'>
                <div className={`p-3 sm:p4 rounded-full bg-gradient-to-br ${item.color}
                transition-transform duration-300 group-hover:scale-110`}>
                    <item.icon className='text-2xl sm:text-3xl text-white'/>

                </div>
                <div className='text-center'>
                    <h3 className='text-x1 sm:text-2xl font-bold font-cursive'>{item.title}</h3>
      ...
      <div className='flex flex-wrap gap-4 items-ceenter mt-6 sm:mt-8 px-4 sm:px-0'>
        <Link to='/about' className='px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-amber-500 to-orange-600
        rounder-xl font-bold  hover:scale-[1.02] transition-transform duration-300 flex item-center gap-2 sm:gap-3 
 ...
 <div
  className="w-full order-2 lg:order-1 md:max-w-md lg:max-w-none lg:w-7/12
  mt-12 mb-10 lg:mb-0 relative group transform
  hover:scale-[1.01] transition-all duration-500"
>
  <div
    className="relative rounded-[4rem] overflow-hidden border-4
    border-amber-900/30 hover:border-amber-600/40
    transition duration-500 shadow-2xl shadow-black/50"
  >
    {/* Gradient overlay */}
    <div
      className="absolute inset-0 bg-gradient-to-r
      from-amber-400/15 via-transparent to-amber-600/40
      mix-blend-soft-light z-10 pointer-events-none"
    ></div>
    ...
    <div
      ...
      w-4/5 h-16 bg-amber-900/30 blur-3xl z-0"
    ></div>

    {/* Top decorative circle */}
    <div
      className="absolute -top-6 -right-6 w-24 h-24
      bg-amber-500/10 rounded-full blur-xl"
    ></div>
  </div>
</div>
