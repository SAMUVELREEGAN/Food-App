...
const Navbar = () => {
    ...
    const [isAuthenticated, setIsAuthenticated] = useState(
        Boolean(localStorage.getItem('lodinDate'))
    )

    useEffect(() => {
        setShowLoginModel(location.pathname === '/login');
        setIsAuthenticated(Boolean(localStorage.getItem('lodinDate')))
    }, [location.pathname])


    const handleLoginSuccess = () => {
        localStorage.setItem('lodinDate', JSON.stringify({ loggedIn: true }));
    ...
    const handleLogout = () => {
        localStorage.removeItem('lodinDate');
    ...
    const renderDesktopAuthButton = () => {
        return isAuthenticated ? (
            <button onClick={handleLogout} className='px-3 md:px-3 lg:px-6 py-1.5 md:py-2 lg:py-3 bg-gradient-to-br from-amber
            to-amber-700 text-[#2D1B0E] rounded-2xl font-blod hover:shadow-lg hover:shadow-amber-600/40 transition-all 
            ...
            <button onClick={() => navigate('/login')}  className='px-3 md:px-3 lg:px-6 py-1.5 md:py-2 lg:py-3 bg-gradient-to-br from-amber
            to-amber-700 text-[#2D1B0E] rounded-2xl font-blod hover:shadow-lg hover:shadow-amber-600/40 transition-all 
            shadow-md shadow-amber-900/20 text-xs md:text-sm lg:text-sm'>
...
// NAVBAR :
  ...
  return (
  <nav
  className="bg-[#2D1B0E] border-b-8 border-amber-900/30 sticky top-0 z-50
  shadow-[0_25px_50px_-12px] shadow-amber-900/30 font-vibes overflow-x-hidden"
>
      ...
      />
        ...
        <div className=' max-w-7xl mx-auto px-4 relative'> 
            <div className='flex justify-between items-center h-16 md:h-20 lg:h-24'>
                ...
                lg:-translate-x-6 ml-0 md:m1-2'>
                    <div className='absolute -inset-4 bg-amber-500/10 rounded-full blur-x1 
                    ...
                    <GiChefToque className='text-3xl md:text-4xl lg:text-5xl text-amber-500 transition-all
                     group-hover:rotate-12 group-hover:text-amber-400 hour:drop-shadow-[0_0_15px] hover:drop-shadow-500/50' />

                    <div className='flex flex-col relative ml-2 max-w-[140px] md:max-w-[160px] lg:max-w-none'>
                        <NavLink to='/' className='text-2xl md:text-xl lg:text-2xl bg-gradient-to-r from-amber-400
                         to-amber-600 bg-clip-text text-transparent front-monsieur tracking-widest drop-shadow-[0_2px_2px] 
                         drop-shadown-back -translate-x-2 truncate md:truncate-none '>
                            Annavasal Hotel
                        </NavLink>
                    ...
                    {/* DESKTOP NAVIGATION */}
                    <div className='hidden md:flex item-center space-x-2 md:space-x-1 lg:space-x-4 flex-1 justify-end'>
                            {navLinks.map((link) => (
                                <NavLink
                                key={link.name}
                                to={link.to}
                                className={({ isActive }) =>
                                 ...
                                 transition-all duration-300 flex item-center hover:bg-amber-900/20 rounded-3x1 border-2 
                                    ${ isActive ? 'border-amber-600/50 bg-amber-900/20 shadow-[inset_0_0_15px] shadow-amber-500/20'
                                        ...
                                        <span className='mr-2 text-sm md:text-[15px] lg:text-base
                                                text-ambar-500 group-hover:text-amber-300 transition-all'>
                                                    {link.icon}
                                        </span>

                                        <span className=' text-amber-100 group-hover:txt-amber-300 relative'>
                                            {link.name}
                            ...
                            md:ml-3 lg:ml-6 mr-2 md:mr-3 lg:mr-4'>
                                <NavLink to='/cart' className='p-2 md:p-2.0 lg:p-3 text-amber-100 rounded-x1 transition-all 
                    ...
                    <div className='md:hidden flex items-center mr-2'>
                        ...
                        p-2 rounded=x1 border-2 border-amber-900/30 hover:border-amber-600/50 relative shadow-md shadow-a
            ...
            {/* MOBILE NAVIGATION */}
            {isOpen && (
                <div className='md:hidden bg-[#2DiB0E] border-t-4 border-amber-900/40 relative shadow-lg
                shadow-amber-900/30 w-full'>
                    <div className='px-4 py-4 space-y-2'>
                        {navLinks.map((link) => (
                            <NavLink key={link.name} to={link.to} onClick={() => setIsOpen(false)} className={({isActive}) =>
                            `block px-4 py-3 text-sm rounded-x1 transition-all items-center ${
  ...
  <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
    <div
      ...
      w-full max-w-[480px] relative border border-amber-700/30
