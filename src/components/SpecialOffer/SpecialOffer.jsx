...
import { FaFire, FaPlus, FaStar } from "react-icons/fa";
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8'>
                {(showAll ? initialData : initialData.slice(0, 4)).map((item, index) => {
                    ...
                    return (
                        ...
                        hover:shadow-red-900/40 border-2 border-transparent hover:border-amber-500 before:absolute 
                        before:inset-0 hover:before:opacity-20'>
                            ...
                            <div className='relative h-72 overflow-hidden'>
                                ...
                                <div className='absolute inset-0 bg-gradient-to-b from-trasparent via-trasparent
                                to-black/90'/>
                                <div className='absolute bottom-4 left-4 right-4 flex justify-between items-center
                                bg-black/40 backdrop-blur-sm px-4 py-2 rounded-full'>
                                     <span className='flex item-center gap-2 text-amber-400'>
                                    <FaStar className='text-xl drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]'/>
                                    <span className='font-bold'>{item.rating}</span>
                                    
                                </span>
                                <span className='flex item-center gap-2 text-red-400'>
                            ...
                            <div className='p-6 relative z-10'>
                                <h3 className='text-2xl fond-bold md-2 bg-gradient-to-r from-amber-300 to-amber-500
                            ...
                            group-hover:border-amber-500/30 transition-all duration-500'></div>
                    ...
                    <div className='mt-12 flex justify-center'>
                        <button onClick={() => setShowAll(!showAll)}
                            className='flex item-center gap-3 bg-gradient-to-r from-red-700 to-amber-700
                            text-white px-8 py-4 rounder-2xl font-bold text-lg uppercase tracking-wider hover:gap-4 hover:scale-105
                            ...
                            overlow-hidden'>
                                ...
                                <div className='h-full w-1 bg-amber-400/30 absolute right-0 top-0 group-hover:animation-border-pulse'></div>
